...
def test_build_dso_index_writes_chroma_collection(tmp_path, fake_embedder):
...
def test_retriever_can_filter_to_university_scope(fake_embedder):
...
def test_retriever_can_limit_to_federal_scope(fake_embedder):
    retriever = HybridDSORetriever.from_documents([
...
def test_retriever_can_mix_federal_and_university_results(fake_embedder):
    retriever = HybridDSORetriever.from_documents([
