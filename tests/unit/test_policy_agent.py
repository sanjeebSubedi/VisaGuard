...
def test_policy_agent_generates_analysis_and_verdict(tmp_path):
    ...
    agent = PolicyAgent(index_path=index_path, cip_dataset_path=DATASET_PATH, llm_client=llm, model_name='test-model')
...
def test_policy_agent_returns_insufficient_evidence_for_unknown_cip(tmp_path):
    ...
    agent = PolicyAgent(index_path=index_path, cip_dataset_path=DATASET_PATH, llm_client=StubLLM([]), model_name='test-model')
    result = agent.evaluate(
        {
            'cip_code': '99.9999',
            'major': 'Unknown Major',
